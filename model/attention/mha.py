...
class Rope_MHA(torch.nn.Module):
    ...
    def forward(self, x, kv_cache=None, past_length=0):
        ...
        ## Apply RoPE
        cos = self.cos_cached[
            :, :, past_length : past_length + S, : self.dh // 2
        ].repeat(1, 1, 1, 2)
        sin = self.sin_cached[
            :, :, past_length : past_length + S, : self.dh // 2
        ].repeat(1, 1, 1, 2)
...
class Decoupled_Rope_MHA(torch.nn.Module):
    def __init__(self, d_model, n_heads, max_len=1024, rope_theta=10000.0):
        ...
        self.wk = torch.nn.Parameter(
            0.01
            * torch.randn((self.n_heads * self.qk_nope_dim) + self.qk_rope_dim, d_model)
        )
        self.wo = torch.nn.Parameter(0.01 * torch.randn((d_model, d_model)))
    ...
    def forward(self, x, kv_cache=None, past_length=0):
        ...
        K, K_for_rope = torch.split(
            K, [self.n_heads * self.qk_nope_dim, self.qk_rope_dim], dim=-1
        )
        ...
        if kv_cache is None:
            ...
            old_K, old_K_for_rope = torch.split(
                k_cache, [self.qk_nope_dim, self.qk_rope_dim], dim=-1
            )

            K = torch.cat([old_K, K], dim=2)
            S_full = K.size(2)

            cos_k = self.cos_cached[
                :, :, past_length : past_length + S, : self.qk_rope_dim // 2
            ].repeat(1, 1, 1, 2)
            sin_k = self.sin_cached[
                :, :, past_length : past_length + S, : self.qk_rope_dim // 2
            ].repeat(1, 1, 1, 2)
            K_for_rope = apply_rope_x(K_for_rope, cos_k, sin_k)
